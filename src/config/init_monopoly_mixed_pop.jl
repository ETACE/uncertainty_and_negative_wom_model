...
baseline_properties = Dict(
    ...
    :s_goldenberg => 0.225,         # fraction of Goldenberg consumers
    :s_bayes => 0.1625,             # fraction of Bayes consumers
    :s_cosita => 0.6125,            # fraction of Cosita consumers
...
function initialize(properties)
    ...
    @assert model.s_goldenberg + model.s_cosita + model.s_bayes > (1-10^-6) && model.s_goldenberg + model.s_cosita + model.s_bayes < (1+10^-6)
    ...
    for i in 1:model.n
        ...
        if r < model.s_goldenberg
            consumer.type = GOLDENBERG
        else
            if r < model.s_goldenberg+model.s_bayes
                consumer.type = BAYES_NORMAL
            else
                consumer.type = COSITA
            end
        end
